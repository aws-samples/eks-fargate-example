...
#Build a tag
if [ -z "$repo" ]; then
  tag="eksfg-efsload:$version"
else
  tag="$repo/eksfg-efsload:$version"
fi

echo $tag

#build image, tag it, and push it to repository
docker build -t eksfg-efsload .
docker tag eksfg-efsload:latest $tag
docker push $tag

#create a K8S manifest to match parameters
sed -i "s/AWS_ACCOUNT/$deploymentAccount/g" ../ETL.yaml
sed -i "s#EFSLOAD_IMAGE_TAG#$tag#g" ../ETL.yaml
